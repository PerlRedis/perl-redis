# This Makefile is for the Redis extension to perl.
#
...
#     BUILD_REQUIRES => { Test::Exception=>q[0], IO::String=>q[0], Test::Fatal=>q[0], Test::Deep=>q[0] }
#     CONFIGURE_REQUIRES => {  }
#     DISTNAME => q[Redis]
#     EXE_FILES => []
#     LICENSE => q[artistic_2]
#     NAME => q[Redis]
#     PREREQ_PM => { Try::Tiny=>q[0], Test::Exception=>q[0], IO::String=>q[0], Test::Fatal=>q[0], Test::Deep=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION => q[1.956]
#     test => { TESTS=>q[t/*.t] }
...
TO_INST_PM = lib/Redis.pm \
	lib/Redis/Hash.pm \
	lib/Redis/List.pm

PM_TO_BLIB = lib/Redis/Hash.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Redis/Hash.pm $(INST_MAN3DIR)/Redis::Hash.$(MAN3EXT) \
	  lib/Redis/List.pm $(INST_MAN3DIR)/Redis::List.$(MAN3EXT) \
	  lib/Redis.pm $(INST_MAN3DIR)/Redis.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Perl binding for Redis database'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Redis' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'version: 1.956' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "Perl binding for Redis database",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Pedro Melo <melo@cpan.org>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Redis",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "1.956"' >> META_new.json
	$(NOECHO) $(ECHO) '}' >> META_new.json
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Redis/Hash.pm blib/lib/Redis/Hash.pm \
	  lib/Redis/List.pm blib/lib/Redis/List.pm \
	  lib/Redis.pm blib/lib/Redis.pm 
