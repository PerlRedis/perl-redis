...
sub redis {
    ...
    if ( my $error = $@ ) {
        reap();
        Test::More::plan(
            skip_all => "Could not start fake redis-server: $error" );
        return;
    }
...
sub spawn_server {
    ...
    if ($pid) {    ## Parent
        sleep 1;
        Test::More::diag("Starting fake server with pid $pid")
          if $ENV{REDIS_DEBUG};
        ...
        my $cleanup = sub {
            ...
            Test::More::diag("Killing fake server at $pid")
              if $ENV{REDIS_DEBUG};
            kill( 15, $pid );
